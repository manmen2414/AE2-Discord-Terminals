...
const Request = (at, data, method) =>
  new Promise(async (s, f) => {
    try {
      const res = await new Promise((resolve, reject) => {
        try {
          const content = !!data ? JSON.stringify(data) : "";
          const req = http.request(process.env.API_URL + at, {
            // <1>
            method: method,
            headers: {
    ...
    Get("/elements")
      .then((json) => {
        ...
        craftableitems.forEach((CAitems) => {
          if (
            items.findIndex((i) => i.name === CAitems.name) === -1 ||
            CAitems.amount === 0
          ) {
            items.push(CAitems);
          }
        ...
        craftablefluids.forEach((CAfluids) => {
          if (
            fluids.findIndex((f) => f.name === CAfluids.name) === -1 ||
            CAfluids.amount === 0
          ) {
        ...
        s(
          items
            .map((e) => ({ type: "item", ...e }))
            .concat(fluids.map((e) => ({ type: "fluid", ...e })))
        );
      })
      .catch((e) => r(e));
  });
