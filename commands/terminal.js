...
const { SlashCommandBuilder, ChatInputCommandInteraction, EmbedBuilder, StringSelectMenuBuilder, StringSelectMenuInteraction,
    ...
    ModalSubmitInteraction, ButtonInteraction, ButtonBuilder, ButtonStyle } = require("discord.js");
...
module.exports = {
    ...
    async SelMenu(SSMInteraction, customId) {
        ...
        const element = elements.find((e) => e.name.split(":")[1] === SSMInteraction.values[0]);
        const modal = new ModalBuilder()
            .setCustomId("gtelcraftmodal" + SSMInteraction.values[0])
            .setTitle(element.displayName + "をクラフトする")
            .addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder()
                .setCustomId("amount").setLabel((element.type === "item" ? "いくつ" : "何mB") + "クラフトする？(半角数字のみ)").setRequired(true).setStyle(TextInputStyle.Short)
            ))
...
class Terminal {
    ...
    createEmbed() {
        ...
        const sortButton = new ButtonBuilder({ customId: "gtelsort", label: "ソート順を" + (sort === "displayName" ? "量が多い順" : "a-z順") + "に切り替える", style: ButtonStyle.Secondary })
        const actionrows = [
        ...
        if (slicedRawCraftSelect.length > 0) {
            actionrows.unshift(new ActionRowBuilder()
        ...
        embed.addFields({
            name: "Items 1",
            value: noMoreNull(FormatedItems.slice(
            ...
            name: "Fluids 1",
            value: noMoreNull(FormatedFluids.slice(
                Terminal.EMBED_LENGTH_PER_PAGE * (this.page),
                Terminal.EMBED_LENGTH_PER_PAGE * (this.page + 1),
            ).join("/n")),
            inline: true
